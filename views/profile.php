...
function getDatabaseConnection() {
    ...
    $dbname = "lelelemon_db"; // Fixed database name to match SQL file
...
if (isset($_POST['update_profile'])) {
    ...
    if (!$error) {
        ...
        if (!$error) {
            // Update user profile - fixed missing bind parameters
            $stmt = $conn->prepare("UPDATE users SET fullname = ?, date_of_birth = ?, email = ?, phone = ?, address = ? WHERE id = ?");
            $stmt->bind_param("sssssi", $fullname, $date_of_birth, $email, $phone, $address, $_SESSION['id']);
...
<body>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Function to adjust the main content based on sidebar state and screen size
