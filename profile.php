...
function getDatabaseConnection() {
    ...
    $dbname = "lelelemon_db"; // Fixed database name to match SQL dump
...
if (isset($_POST['update_profile'])) {
    ...
    if (!$error) {
        ...
        if (!$error) {
            // Fixed the parameter binding issue - corrected parameters and count
            $stmt = $conn->prepare("UPDATE users SET email = ?, phone = ?, address = ? WHERE id = ?");
            $stmt->bind_param("sssi", $email, $phone, $address, $_SESSION['id']);
...
// Include sidebar before the DOCTYPE to ensure proper structure
...
<head>
  ...
  body {
    ...
    padding-left: 88px; /* Default sidebar width (when closed) */
  }

  body.sidebar-expanded {
    padding-left: 250px; /* Expanded sidebar width */
  }
...
<body>
    ...
    <div class="row">
      ...
      <div class="col-md-4 mb-4">
        <div class="profile-card">
          ...
          <div class="headings">
            <p class="fs-4 fw-bold text-dark mb-0"><?php echo htmlspecialchars($full_name); ?></p>
            <p class="text-muted"><?php echo htmlspecialchars(isset($_SESSION["role"]) ? ucfirst($_SESSION["role"]) : "User"); ?></p>
          </div>
          <div class="contact-info">
            <ul>
              <li>
                <i class='bx bxs-phone'></i>
                <span><?php echo htmlspecialchars($_SESSION["phone"] ?? "Not set"); ?></span>
              </li>
              <li>
                <i class='bx bxs-envelope'></i>
                <span><?php echo htmlspecialchars($_SESSION["email"] ?? "Not set"); ?></span>
              </li>
              <li>
                <i class='bx bxs-map'></i>
                <span><?php echo htmlspecialchars($_SESSION["address"] ?? "Not set"); ?></span>
              </li>
            </ul>
          </div>
  ...
  // Handle sidebar toggle to adjust main content area
